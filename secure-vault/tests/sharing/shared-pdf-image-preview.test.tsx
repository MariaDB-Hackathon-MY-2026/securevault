...
describe("SharedPdfImagePreview", () => {
  ...
  it("calls the manifest route and renders page image urls", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("shared-pdf-preview-page-image-1").getAttribute("src")).toBe(
        "/api/share/share-token/pdf-preview/pages/1",
      );
      expect(screen.getByTestId("shared-pdf-preview-page-image-2").getAttribute("src")).toBe(
        "/api/share/share-token/pdf-preview/pages/2",
      );
    });
  });
