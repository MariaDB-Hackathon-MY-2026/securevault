...
CREATE TABLE `embedding_chunks` (
	...
	`modality` enum('pdf','image') NOT NULL DEFAULT 'pdf',
	`page_from` int,
	`page_to` int,
	`char_count` int,
	`encrypted_text` longblob,
	`text_iv` blob,
	`text_auth_tag` blob,
	`embedding` vector(1536) NOT NULL,
	CONSTRAINT `embedding_chunks_id` PRIMARY KEY(`id`),
	CONSTRAINT `uq_embedding_chunks_job_chunk` UNIQUE(`job_id`,`chunk_index`)
);
--> statement-breakpoint
CREATE TABLE `embedding_jobs` (
	...
	CONSTRAINT `embedding_jobs_id` PRIMARY KEY(`id`),
	CONSTRAINT `uq_embedding_jobs_file_modality` UNIQUE(`file_id`,`modality`)
);
--> statement-breakpoint
...
ALTER TABLE `embedding_chunks` ADD CONSTRAINT `embedding_chunks_job_id_embedding_jobs_id_fk` FOREIGN KEY (`job_id`) REFERENCES `embedding_jobs`(`id`) ON DELETE cascade ON UPDATE cascade;--> statement-breakpoint
ALTER TABLE `embedding_chunks` ADD CONSTRAINT `embedding_chunks_file_id_files_id_fk` FOREIGN KEY (`file_id`) REFERENCES `files`(`id`) ON DELETE cascade ON UPDATE cascade;--> statement-breakpoint
ALTER TABLE `embedding_jobs` ADD CONSTRAINT `embedding_jobs_file_id_files_id_fk` FOREIGN KEY (`file_id`) REFERENCES `files`(`id`) ON DELETE cascade ON UPDATE cascade;--> statement-breakpoint
ALTER TABLE `embedding_jobs` ADD CONSTRAINT `embedding_jobs_triggered_by_users_id_fk` FOREIGN KEY (`triggered_by`) REFERENCES `users`(`id`) ON DELETE set null ON UPDATE cascade;--> statement-breakpoint
...
CREATE INDEX `idx_embedding_chunks_file_id` ON `embedding_chunks` (`file_id`);--> statement-breakpoint
...
CREATE INDEX `idx_upload_sessions_user_file` ON `upload_sessions` (`user_id`,`file_name`,`file_size`,`status`);