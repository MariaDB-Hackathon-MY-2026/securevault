...
| **HKDF** (RFC 5869)               | Derive a separate key for encrypted extracted text | [RFC 5869](https://www.rfc-editor.org/rfc/rfc5869) |
...
| **OWASP Cheat Sheet: File Upload**           | Upload validation and MIME handling | [Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html) |
| **OWASP Cheat Sheet: Secrets Management**    | Protect API keys for embedding/OCR providers | [Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Secrets_Management_Cheat_Sheet.html) |
...
## Derived Content Guidance

- Encrypt extracted PDF chunk text at rest with a key derived from the file FEK; do not store searchable snippets in plaintext.
- Treat embeddings as derived metadata that remains queryable in MariaDB vector columns and protect access through auth-scoped queries.
- Keep OCR and embedding provider credentials in server-side env vars only.
- Document which providers may receive decrypted PDF content during indexing and keep that data flow out of the normal upload critical path.
