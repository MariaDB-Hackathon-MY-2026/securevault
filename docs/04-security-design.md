...
### Derived PDF Index Artifacts

- Extracted PDF chunk text is encrypted at rest using a key derived from the file FEK
- Semantic embeddings remain plaintext vectors in MariaDB because vector indexing requires searchable numeric values
- Only auth-scoped services may decrypt snippets for semantic search results
- OCR and embedding providers never receive MK, UEK, or raw encryption metadata
...
| Derived text leak | Encrypt extracted text chunks with FEK-derived key |
| OCR provider leak | Send only decrypted PDF/page data needed for indexing |
| Vector query leak | Scope semantic search by current user before ranking |
...
| PDF indexing  | Only `application/pdf` and only when `size <= 10MB` |
...
Semantic indexing should also be verified for:

- Upload-ready status remaining intact when OCR or embedding fails
- PDFs over 10MB being skipped instead of rejected from normal storage
- Semantic search returning only results for the authenticated user
